...
namespace mynum
{
  namespace f
  {
    ...
    float_t coth(const float_t &x, const float_t &epsilon, size_t precision)
    {
      //
      // 保证 0< |x| < pi
      //
      ...
      float_t y = const_1, sign = mympf::create("-1");
      ...
      do
      {
        ...
        numerator = pow(l, exponent, epsilon, precision);
        ...
        item = mympf::mul(item, pow(sign, mympf::sub(n, o), epsilon, precision));
        y = mympf::add(y, item);
